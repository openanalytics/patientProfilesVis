...
```{r loadData}	
	...
	dataRes <- loadDataADaM(dataFiles)
	data <- dataRes
	labelVars <- attr(dataRes, "labelVars")

```

# Create the plot submodules
...
#	subset(params, grepl("leuk", PARAM, ignore.case = TRUE))
	...
	dataLB <- subset(data$ADLB, 
		PARAMCD %in% 
			c("MONO", "LYM", "BASO", "WBC", "PLAT", "ALT", "AST", "ALP", "GGT") &
		LBTEST != "" &
...
```{r lb-spaghettiPlot}
	...
	# create plot
	lbPlotsSpagh <- subjectProfileSpaghettiPlot(
		data = dataLB,
		paramVar = "AVAL",
		facetVar = "LBTESTCD",
		timeVar = "LBDY",
		labelVars = labelVars,
		title = "Laboratory test measurements: spaghetti plots"
	)
...
```{r exampleTestModule-wideFormat-include, echo = FALSE}
	...
	tempVsPlots <- subjectProfileSpaghettiPlot(
		data = dataVSTemp,
		paramVar = "AVAL",
		facetVar = "VSTEST",
		timeVar = "ADY",
...
```{r createReportWithAllModules-referenceLinesFromSAP, eval = FALSE}
	...
	createSubjectProfileReport(
		...
		outputFile = "subjectProfile_1972-CL-101.pdf",
		maxNLines = 100
	)
