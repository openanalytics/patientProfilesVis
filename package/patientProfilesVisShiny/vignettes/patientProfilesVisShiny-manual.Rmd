...
**Time variable should be numeric, and consistent across the different
event/interval/line modules, e.g. start/end, study day...**
...
* `Select subjects based on:`: filter subjects to consider in final
  report based on:
    + `Dataset`: dataset containing subjects of interest.
    If `Variable` is not specified, only subjects present in this dataset are
      considered (e.g. to consider only subjects with adverse events, select
      the adverse events dataset).
    + `Variable`: variable used to select the subjects on (in combination
      with `Group(s)`).
    + `Group(s)`: group(s) indicated in `Variable` to consider. For example,
      to select only the treated subjects, choose 'ARM' in
      `Variable` and corresponding treatment label in 'Group(s)' of
      interest.
