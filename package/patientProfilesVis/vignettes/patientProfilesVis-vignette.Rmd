...
```{r exampleIntervalModule-exinclude, echo = FALSE, fig.height = getNLinesYGgplot(exPlots[[1]])*0.14, fig.width = 14, fig.cap = paste("Exposure interval with the 'subjectProfileIntervalPlot' function for patient:", names(aePlots)[1])}
...
The reference lines are extracted from the subject visits: `SV` dataset.
...
```{r createReportFromADAMDataset, eval = FALSE, echo = FALSE}
	...
	lbPlotsADaM <- subjectProfileEventPlot(
		...
		labelVars = labelVarsSDTMPelican,
		shapeVar = "ANRIND",
		shapePalette = c('L' = 25, 'N' = 19, 'H' = 24, 'H+L' = 23)
	)
	...
	createSubjectProfileReport(
		...
		refLinesData = ADaMDataPelican$ADSL,
		refLines = refLinesParam,
		outputFile = "subjectProfile_SDTM_referenceLinesFromSAP.pdf"
	)
