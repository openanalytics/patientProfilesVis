...
By default, **missing start/end values are represented as an arrow, positioned at the minimum/maximum
time value per subject**.
To set the values represented for records with missing start/end dates,
the **time limits can be specified** via the **`timeLim`** parameter
or extracted from a **specified dataset containing the start/end date for each
subject** via the **`timeLimData` and `timeLimVar`** parameters.

```{r exampleIntervalModule-ae-timeLim}

	aePlotsTimLim <- subjectProfileIntervalPlot(
		data = dataAE,
		paramVar = "AETERM",
		timeStartVar = "AESTDY",
		timeEndVar = "AEENDY",
...
```{r exampleIntervalModule-ae-timeLim-include, echo = FALSE, fig.height = attributes(aePlotsTimLim[[1]][[1]])$metaData$nLines*heightLineIn, fig.width = 14, fig.cap = paste0("Adverse events with the 'subjectProfileIntervalPlot' function for patient:", names(aePlotsTimLimFromSV)[1], ". Missing start/end date are extracted from the specified time limits.")}

	print(aePlotsTimLim[[1]][[1]])

```

The second option is used below to extract the time limits based on all visits for a specific subject from the subject-level SDTM dataset.
...
specific subject is used, and across subjects if all missing. This might result
...
Please note that because the default `ggplot2` symbol palette
...
```{r exampleIntervalModule-ae-customMissingPartialDates-include, echo = FALSE, fig.height = attributes(aePlotsShape[[1]][[1]])$metaData$nLines*heightLineIn, fig.width = 14, fig.cap = paste("Adverse events with the 'subjectProfileIntervalPlot' function for patient:", names(aePlotsShape)[1], "with custom shape specification")}

	print(aePlotsShape[[1]][[1]])
...
```{r exampleIntervalModule-cm-timeAlign-FALSE-include-1, echo = FALSE, fig.height = attributes(cmPlotsNotAligned[[2]][[1]])$metaData$nLines*heightLineIn, fig.width = 14, fig.cap = paste("Adverse events with the 'subjectProfileIntervalPlot' function for patient:", names(cmPlotsNotAligned)[2], "with custom shape specification")}

	print(cmPlotsNotAligned[[2]][[1]])

```

Please note that if all start/end times are missing for one subject,
the minimum/maximum is taken across subjects (see section [#intervalMissingStartEnd]
on how to customize the specification of time limits).

```{r exampleIntervalModule-cm-timeAlign-FALSE-include-2, echo = FALSE, fig.height = attributes(cmPlotsNotAligned[[1]][[1]])$metaData$nLines*heightLineIn, fig.width = 14, fig.cap = paste("Adverse events with the 'subjectProfileIntervalPlot' function for patient:", names(cmPlotsNotAligned)[1], "with custom shape specification")}

	print(cmPlotsNotAligned[[1]][[1]])
...
* 'asinh': hyperbolic arc-sine transformation, to zoom in small time values (around 0).
...
the `createSubjectProfileReport` should be called
with the parameter `timeAlign` set to `FALSE`.
...
```{r createReportWithAllModules, eval = FALSE}
	...
	# extract patients with CM starting early/late
	CMWithStartEnd <- subset(SDTMDataPelican$CM, !is.na(CMSTDY))
	subjectSubset <- CMWithStartEnd[c(which.max(CMWithStartEnd$CMSTDY), which.min(CMWithStartEnd$CMSTDY)), "USUBJID"]
	
	# create report
	createSubjectProfileReport(
		# general
		listPlots = list(dmPlots, cmPlotsNotAligned, exPlots, aePlots, lbPlotsColor),
		subset = subjectSubset,
		timeAlign = FALSE,
		outputFile = "subjectProfile_SDTM_referenceLinesFromSV_nonAlign.pdf",
		# reference line(s)
		refLinesData = SDTMDataPelican$SV,
		refLinesTimeVar = "SVSTDY",
		refLinesLabelVar = "VISIT",
