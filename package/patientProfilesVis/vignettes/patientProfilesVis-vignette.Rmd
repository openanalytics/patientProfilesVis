...
Records with missing starting/end time are imputed for the visualization.
By default, specific symbols are used to represent missing starting/end time.
...
`subjectProfileIntervalPlot` function for the most up-to-date information on 
this imputation**.

```{r exampleIntervalModule-ae-default}
...
```{r exampleIntervalModule-ae-customMissingPartialDates-include, echo = FALSE, fig.height = attributes(aePlotsShape[[subjectAE]][[1]])$metaData$nLines*heightLineIn, fig.width = 14, fig.cap = paste("Adverse events with the 'subjectProfileIntervalPlot' function for patient:", subjectAE, "with custom shape specification")}
