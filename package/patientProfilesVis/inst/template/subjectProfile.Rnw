...
<<setMargins>>=
	cat(paste0("\\usepackage[margin=", inputParameters$margin, "in]{geometry}\n"))
@
...
<<includePlots>>=
	...
	figHeightInches <- sapply(listPlotsPerSubject, function(plotSubject)
		...
		attr(plotSubject, "nLinesPlot") * inputParameters$heightLineIn
	)
	...
	# extract titles
	sectionTitles <- unlist(lapply(names(listPlotsPerSubjectList), function(name) 
		c(name, rep("", length(listPlotsPerSubjectList[[name]])-1))))
