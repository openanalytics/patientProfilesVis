...
#			linetype = "missingStartEndPlot"
		...
		gg <- gg + geomSegmentCustom(data = dataSubject, show.legend = TRUE)	
		...
		dataPlotByME <- dlply(dataSubject, c("missingStartPlot", "missingEndPlot"))
...
formatTimeInterval <- function(data, 
	...
	res <- list(data = data, timeLim = timeLim)
